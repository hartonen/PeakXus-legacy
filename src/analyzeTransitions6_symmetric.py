...
#CONSTRUCTOR
    ...
    def calcTransitions(self,chrom,reads,allpairs,nproc):
        ...
        self.read_counts_plus[chrom] = reads.getAllReadHisto(chrom,'++',self.s,nproc)
        #self.read_counts_plus[chrom] = reads.getReadHisto(chrom,'++',self.s,nproc)
        self.read_counts_minus[chrom] = reads.getAllReadHisto(chrom,'--',self.s,nproc)        
        #self.read_counts_minus[chrom] = reads.getReadHisto(chrom,'--',self.s,nproc)        
        ...
        #rdinate is the coordinate of the -
        ...
        for k in range(1,len(keys)):
            ...
            if self.read_counts_plus[chrom][i-self.l_limit:i-self.u_limit].max()<1: continue
            ...
            if allpairs==1:
                ...
                aux_j = self.read_counts_plus[chrom][i-self.l_limit/2:i-1].argmax()+i-(self.l_limit/2+1)
    ...
    def testBackground(self,chrom,p_threshold,yates,nproc,pseudo):
        ...
        for k in range(0,len(res)):
            if res[k][0] == True: self.tPoints[chrom][k,-1] = -1
                ...
                j = self.tPoints[chrom][k,0]
                i = self.tPoints[chrom][k,1]
                if yates==0:
                    #G-test
        ...
        if len(self.tPoints[chrom])>0: self.delOverlap()
    #end testBackground
    ...
    def delOverlap(self):
        ...
        for c in self.tPoints:
            for i in range(1,len(self.tPoints[c])):
                ...
                current_score = self.tPoints[c][i,-1]
                ...
                while j>=0:
                    j -= 1
                    if self.tPoints[c][j,-1]==-1: continue
                    ...
                    candidate_score = self.tPoints[c][j,-1]
                ...
                for d in deleted: self.tPoints[c][d,-1] = -1
            
            #deleting the high p-value overlapping peak candidates from chromosome c
            self.tPoints[c] = self.tPoints[c][np.where(self.tPoints[c][:,-1]>-1)[0],:]
